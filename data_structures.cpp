...
template <typename T>
shared_ptr<circular_singly_linked_list::Csll_node<T>> circular_singly_linked_list::Csll<T>::front(){
    return head_pointer;
}

template <typename T>
shared_ptr<circular_singly_linked_list::Csll_node<T>> circular_singly_linked_list::Csll<T>::back(){
    return tail_pointer;
}

template <typename T>
void circular_singly_linked_list::Csll<T>::push_back( const T& element ) {
    shared_ptr<Node> new_node = make_shared<Node>();
    new_node->value = element; new_node->next_node = head_pointer;
    if (head_pointer == nullptr) {
        tail_pointer = head_pointer = new_node;
...
template <typename T>
void circular_singly_linked_list::Csll<T>::push_front( const T& element ) {
    shared_ptr<Node> new_node = make_shared<Node>();
    new_node->value = element; new_node->next_node = head_pointer;
    if ( head_pointer == nullptr ) {
        head_pointer = tail_pointer = new_node;
...
template <typename T>
shared_ptr<circular_singly_linked_list::Csll_node<T>> circular_singly_linked_list::Csll<T>::pop_back() {
    if ( head_pointer == tail_pointer ){
        shared_ptr<Node> tmp = head_pointer;
        // delete head_pointer;
        size--;
...
template <typename T>
shared_ptr<circular_singly_linked_list::Csll_node<T>> circular_singly_linked_list::Csll<T>::pop_front() {
    shared_ptr<Node> tmp = head_pointer;
    if ( head_pointer == tail_pointer ){
        // delete head_pointer;
        head_pointer = tail_pointer = nullptr;
...
template <typename T>
shared_ptr<circular_singly_linked_list::Csll_node<T>> circular_singly_linked_list::Csll<T>::operator[] ( const int& place) {
    shared_ptr<Node> pointer = head_pointer;
    if( place == 0 ) { return front(); }
    else if ( place <= get_size() ) {
        for (auto i = 0; i != place+1; i++) {
...
template <typename T>
bool circular_singly_linked_list::Csll<T>::empty() {
    return head_pointer == nullptr;
}

template <typename T>
unsigned int circular_singly_linked_list::Csll<T>::get_size() {
    return size;
}

template <typename T>
void circular_singly_linked_list::Csll<T>::clear() {
    while( !empty() ) {
        pop_back();
    }
}
...
template <typename T>
void circular_singly_linked_list::Csll<T>::erase( shared_ptr<Node> pointer ) {
    if( pointer == head_pointer ) {
        pop_front();
    } else if ( pointer == tail_pointer ) {
        pop_back();
    } else {
        for (shared_ptr<Node> tmp = head_pointer; tmp != tail_pointer; tmp = tmp->next_node){
            if ( tmp->next_node == pointer ) {
                tmp->next_node = ( tmp->next_node )->next_node;
                // delete pointer;
                size--;
...
template <typename T>
void circular_singly_linked_list::Csll<T>::insert( shared_ptr<Node> pointer, const T& element ) {
    if( pointer == head_pointer ) {
        push_front(element);
    } else if ( pointer == tail_pointer->next_node ) {
        push_back(element);
...
template <typename T>
void circular_singly_linked_list::Csll<T>::remove( const T& element ) {
    shared_ptr<Node> tmp = head_pointer;
    shared_ptr<Node> ptr;
    do{
        if( element == tmp->value ) {
...
template <typename T>
circular_singly_linked_list::Csll<T>& circular_singly_linked_list::Csll<T>::sort() {
    for( auto i = 1 ; i < get_size() ; i++ ) {
        T it = ( ( *this )[i] )->value;
        int j = i - 1;
        while(j >= 0 && ( ( *this )[j] )->value > it) {
...
template <typename T>
circular_singly_linked_list::Csll<T>& circular_singly_linked_list::Csll<T>::reverse() {
    for( auto i = 0; i < get_size()/2; i++ ) {
        T tmp = ( ( *this )[i] )->value;
        ( ( *this )[i] )->value = ( ( *this )[size-1-i] )->value;
        ( (*this)[size-1-i] )->value = tmp;
...
template <typename T>
shared_ptr<circular_doubly_linked_list::Cdll_node<T>> circular_doubly_linked_list::Cdll<T>::front(){
    return head_pointer;
}

template <typename T>
shared_ptr<circular_doubly_linked_list::Cdll_node<T>> circular_doubly_linked_list::Cdll<T>::back(){
    return tail_pointer;
}

template <typename T>
void circular_doubly_linked_list::Cdll<T>::push_back( const T& element ) {
    if ( head_pointer == nullptr ) {
        shared_ptr<Node> new_node = make_shared<Node>();
        new_node->value = element; new_node->prev_node = nullptr; new_node->next_node = nullptr;
        head_pointer = tail_pointer = new_node;
...
template <typename T>
void circular_doubly_linked_list::Cdll<T>::push_front( const T& element ) {
    if ( head_pointer == nullptr ) {
        shared_ptr<Node> new_node = make_shared<Node>();
        new_node->value = element; new_node->prev_node = nullptr; new_node->next_node = nullptr;
        head_pointer = tail_pointer = new_node;
...
template <typename T>
shared_ptr<circular_doubly_linked_list::Cdll_node<T>> circular_doubly_linked_list::Cdll<T>::pop_back() {
    if ( head_pointer == tail_pointer ){
        shared_ptr<Node> tmp = head_pointer;
        // delete tmp;
        size--;
...
template <typename T>
shared_ptr<circular_doubly_linked_list::Cdll_node<T>> circular_doubly_linked_list::Cdll<T>::pop_front() {
    if ( head_pointer == tail_pointer ){
        shared_ptr<Node> tmp = head_pointer;
        // delete head_pointer;
        size--;
...
template <typename T>
shared_ptr<circular_doubly_linked_list::Cdll_node<T>> circular_doubly_linked_list::Cdll<T>::operator[] ( const int& place) {
    shared_ptr<Node> pointer = head_pointer;
    if( place == 0 ) { return front(); }
    else if ( place <= get_size() ) {
        for (auto i = 0; i != place+1; i++) {
...
template <typename T>
bool circular_doubly_linked_list::Cdll<T>::empty() {
    return head_pointer == nullptr;
}

template <typename T>
unsigned int circular_doubly_linked_list::Cdll<T>::get_size() {
    return size;
}

template <typename T>
void circular_doubly_linked_list::Cdll<T>::clear() {
    while( !empty() ) {
        pop_back();
    }
}
...
template <typename T>
void circular_doubly_linked_list::Cdll<T>::erase( shared_ptr<Node> pointer ) {
    if( pointer == head_pointer ) {
        pop_front();
    } else if ( pointer == tail_pointer ) {
        pop_back();
...
template <typename T>
void circular_doubly_linked_list::Cdll<T>::insert( shared_ptr<Node> pointer, const T& element ) {
    if( pointer == head_pointer ) {
        push_front(element);
    } else if ( pointer == tail_pointer->next_node ) {
        push_back(element);
...
template <typename T>
void circular_doubly_linked_list::Cdll<T>::remove( const T& element ) {
    if( element == head_pointer->value ) {
        pop_front();
    } else {
        shared_ptr<Node> tmp = head_pointer;
...
template <typename T>
circular_doubly_linked_list::Cdll<T>& circular_doubly_linked_list::Cdll<T>::sort() {
    for(auto i = 1 ; i < get_size() ; i++) {
        T it = ( ( *this )[i] )->value;
        int j = i - 1;
        while(j >= 0 && ( ( *this )[j] )->value > it) {
...
template <typename T>
circular_doubly_linked_list::Cdll<T>& circular_doubly_linked_list::Cdll<T>::reverse() {
    for(auto i = 0; i < get_size()/2; i++) {
        T tmp = ( ( *this )[i] )->value;
        ( ( *this )[i] )->value = ( ( *this )[size-1-i] )->value;
        ( (*this)[size-1-i] )->value = tmp;
